...
-- Add a flag so admins can exclude an article from trending rotation
ALTER TABLE public.articles ADD COLUMN IF NOT EXISTS trending_excluded boolean DEFAULT false;

-- Update the rotation function to skip excluded articles
...
BEGIN
  ...
  -- B) Clear trending on non-pinned, non-excluded articles
  UPDATE articles
  SET is_trending = FALSE, trending_rotated_at = NOW()
  WHERE is_trending = TRUE
    AND (featured_pinned IS NOT TRUE)
    AND (trending_excluded IS NOT TRUE);

  -- C) Set trending on top 10 globally (last 180 days), excluding manually excluded articles
  UPDATE articles
