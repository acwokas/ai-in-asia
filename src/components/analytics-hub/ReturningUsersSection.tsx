  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      pageviews.forEach(pv => {
        if (!pageSessionMap[pv.page_path]) pageSessionMap[pv.page_path] = new Set();
        pageSessionMap[pv.page_path].add(pv.session_id);
      });
              ...
              <TableBody>
                {data.topRevisited.map(p => (
                  <TableRow key={p.path}>
                    ...
                    <TableCell className="text-right font-medium">{p.uniqueSessions}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
